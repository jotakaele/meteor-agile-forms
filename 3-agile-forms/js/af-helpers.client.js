...
insertDataValues = function insertDataValues(form, data) {
        ...
        _(data).each(function(value, key, theR) { //por cada item en data
            if (_.has(form, key)) { //Si existe la clave en el form
                ...
                if (s.startsWith(key, '_')) { //Si comienza por _
...
bdToHtmlValue = function bdToHtmlValue(value, typeHTML) {
        switch ($.type(value)) {
            ...
            case 'date':
                switch (typeHTML) {
                    case 'date':
                        res = moment(value).format(se('default_date_format').moment)
                        break;
                    case 'datetime':
                        res = moment(value).format(se('default_datetime_format').moment)
                        break;
                    case 'time':
                        res = moment(value).format(se('default_time_format').moment)
                        break;
                    default:
                        break;
                }
