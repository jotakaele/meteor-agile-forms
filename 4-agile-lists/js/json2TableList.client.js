    ...
    function parseJson(dataArray, dataDest) {
            ...
            while ($("div[type=object]", theTable).length >= 1) {
                $("div[type=object]", theTable).each(function () {
                    ...
                    var objLev = $(this).closest("[type=renderObject]").attr("objectlevel") || 0
                    $(this).attr('objectlevel', $(this).parents('[objectlevel]').length)
                    $(this).removeAttr("source")
                })
            }
            //        $('[objectlevel]').each(function () {
            //  var parentlevel = $(this).closest('[objectlevel]').attr('objectlevel')
            //  $(this).attr('objectlevel', $(this).parents('[objectlevel]').length)
            //})
